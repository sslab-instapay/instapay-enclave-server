...
App_Cpp_Files := $(shell find untrusted/ -type f -name '*.cpp')
App_Include_Paths := -Iinclude -Iuntrusted -I$(SGX_SDK)/include
...
Enclave_Cpp_Flags := $(Enclave_Common_Flags) -nostdinc++ -std=c++11
...
enclave/%.o: enclave/%.c enclave/enclave_t.h
	@$(CC) $(SGX_COMMON_CXXFLAGS) $(Enclave_C_Flags) -c $< -o $@
	@echo "CXX  <=  $<"
